...
def get_crt(account_key, csr, acme_dir, log=LOGGER, CA=DEFAULT_CA, disable_check=False,
    ...
    def _cmd(cmd_list, stdin=None, cmd_input=None, err_msg="Command Line Error"):
        proc = subprocess.Popen(cmd_list, stdin=stdin, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
    ...
    log.info("Account key dir %s " % (account_key,))
    out = _cmd(["openssl", "rsa", "-in", account_key, "-noout", "-text"], err_msg="OpenSSL Error")
    # pub_pattern = r"modulus:\n\s+00:([a-f0-9\:\s]+?)\npublicExponent: ([0-9]+)" for ubuntu
    pub_pattern = r"modulus:\r\n\s+00:([a-f0-9\:\s]+?)\r\npublicExponent: ([0-9]+)" # for windows
